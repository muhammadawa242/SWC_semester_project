...
app.get("/profile", (req, res) => {
	...
	res.send(`<h1>${req.cookies.auth}</h1>`);
	if (token) {
		jwt.verify(token, jwtSecret, {}, (err, userData) => {
			if (err) throw err;
			res.json(userData);
		...
		res.status(401).json("no token why you tripping");
	}
});

app.get("/people", async (req, res) => {
	const users = await User.find({}, { _id: 1, firstName: 1 }); //this is a promise
	res.json(users);
});
...
app.post("/login", async (req, res) => {
	...
	if (foundUser) {
		...
		if (passOk) {
			jwt.sign(
				{ userId: foundUser._id, firstName },
				jwtSecret,
				{},
				(err, token) => {
...
app.post("/register", async (req, res) => {
	...
	try {
		...
		jwt.sign(
			{ userId: createdUser._id, firstName },
			jwtSecret,
			{},
			(err, token) => {
				if (err) throw err;
